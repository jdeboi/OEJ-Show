...
public void mouseEvent(MouseEvent e) {
	PApplet.println("keystone mouseEvent");
	// ignore input events if the calibrate flag is not set
	if (!calibrate) {
		PApplet.println("not calibrating; returning");
		return;
	}
	...
	switch (e.getAction()) {
		...
		for (int i=surfaces.size()-1; i >= 0; i--) {
			...
			if (dragged != null) {
				PApplet.println("we got a dragged != null in mouse press");
				top = s;
				break;
			}
		}
		...
		if (dragged != null)
		PApplet.println("we dragging");
		dragged.moveTo(x, y);
		break;

		case MouseEvent.RELEASE:
		PApplet.println("we releasing");
		dragged = null;
		break;
	}
}
